...
def callbackinspected(data):
	...
	ws_lock.release()
	rospy.loginfo('event propagated to oracle')
def callback_move_base_goal(data):
	global ws, ws_lock
	rospy.loginfo('monitor has observed: ' + str(data))
	dict = message_converter.convert_ros_message_to_dictionary(data)
	dict['topic'] = '/move_base/goal'
	dict['time'] = rospy.get_time()
	ws_lock.acquire()
	while dict['time'] in dict_msgs:
		dict['time'] += 0.01
	...
	rospy.loginfo('event propagated to oracle')
pub_dict = {}
msg_dict = { '/radiation_sensor_plugin/sensor_0' : "gazebo_radiation_plugins/Simulated_Radiation_Msg",  '/command' : "gazebo_radiation_plugins/Command",  'inspected' : "gazebo_radiation_plugins/Inspection",  '/move_base/goal' : "move_base_msgs/MoveBaseActionGoal"}
def monitor():
	global pub_error, pub_verdict
	with open(log, 'w') as log_file:
		log_file.write('')
	rospy.init_node('rvaft_monitor', anonymous=True)
	pub_error = rospy.Publisher(name = 'rvaft_monitor/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
	pub_verdict = rospy.Publisher(name = 'rvaft_monitor/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
	...
	rospy.Subscriber('/move_base/goal', MoveBaseActionGoal, callback_move_base_goal)
...
def main(argv):
	...
	log = '/media/angelo/WorkData/git/radiation_ws/src/monitor/log_rvaft.txt' 
	actions = {
		'/radiation_sensor_plugin/sensor_0' : ('log', 1), 
		'/command' : ('log', 1), 
		'inspected' : ('log', 1), 
		'/move_base/goal' : ('log', 1)
	}
	monitor()
